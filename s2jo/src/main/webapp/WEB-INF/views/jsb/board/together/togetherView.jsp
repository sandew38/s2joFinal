...
function goLikeDisLikeCountShow() {
	...
	$.ajax({
		url: "/khx/jsb/tlikeDislike.action",
		type: "POST",
		data: form_data,
		dataType: "JSON",
...
function godislikeAdd(seq, userid) {
	...
	$.ajax({
		url: "/khx/jsb/tdislikeAdd.action",
		type: "POST",
		data: form_data,
		dataType: "JSON",
		success: function(data){
...
<script type="text/javascript">
	function goDelete() {
		...
		delFrm.action = "/khx/jsb/togetherDel.action";
		delFrm.method = "post";
		delFrm.submit();
	}
</script>
...
<div style="min-height:100%; background-color: white; border: solid 0px red;">
	...
	<table id="tablecontent1"class="table table-condensed"> 
		<tr>
           	<td><span style="font-weight: bold;">[함께해요게시판]${boardvo.subject}</span></td>
        </tr>
		<tr>
			...
			<td  align="right">			
			<a href="<%= request.getContextPath() %>/jsb/togDownload.action?seq=${boardvo.seq}"> 
	...
	<br/>	
	<div align="right">
		<button type="button" class="btn " onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/togetherList.action'">목록보기</button>
		<button type="button" class="btn " onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/togetherEdit.action?seq=${boardvo.seq}&writer=${boardvo.userid}'">수정</button>
	...
	<!--  댓글쓰기 폼 추가 ===== -->
	<form name="addWriteFrm" action="<%= request.getContextPath() %>/jsb/togAddComment.action" method="get">  
