...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#search").keyup(function(){
	    	...
	    	$.ajax({
	    		url: "/khx/jsb/worWordSearchShow.action",
	    		type: "GET",
	...
	function goSearch(){
		...
		if(search.trim() == "") {
			...
			searchFrm.action = "/khx/jsb/worryingList.action";
...
<br/>	
<br/>
	<span style="font-size:28px; color: #00AF87; font-weight: bold; ">주의해요 </span><span style="font-size:24px;  ">글 목록</span>
			...
			<tr>
				<td align="center">${boardvo.seq}</td>
	             ...
	             <!-- 답변글이 아닌 원글인 경우 -->
					 ...
					 <a href="<%= request.getContextPath() %>/jsb/worryingView.action?seq=${boardvo.seq}&writer=${boardvo.userid}">${boardvo.subject}<span style="color: red; font-weight: bold; font-style: italic; font-size: smaller; vertical-align: super; ">[${boardvo.commentCount}]</span></a>
					
					 </c:if>
					 
					 <c:if test="${boardvo.commentCount == 0}">
					 <a href="<%= request.getContextPath() %>/jsb/worryingView.action?seq=${boardvo.seq}&writer=${boardvo.userid}">${boardvo.subject}</a>
				...
				<td align="center">${boardvo.wLikeCnt }</td>
				<td align="center">${boardvo.wDisLikeCnt }</td>
	...
	<button class="btn" type="button" onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/worryingList.action'">글목록</button>&nbsp;
	<button class="btn" type="button" onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/worryingAdd.action'">글쓰기</button>
