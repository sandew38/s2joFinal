...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#search").keyup(function(){
	    	...
	    	$.ajax({
	    		url: "/khx/jsb/recWordSearchShow.action",
	    		type: "GET",
	...
	function goSearch(){
		...
		if(search.trim() == "") {
			...
			searchFrm.action = "/khx/jsb/recommendList.action";
...
<br/>	
<br/>
	<span style="font-size:28px; color: #00AF87; font-weight: bold; ">추천해요 </span><span style="font-size:24px;  ">글 목록</span>
			...
			<tr>
				<td align="center">${boardvo.seq}</td>
	             ...
	             <!-- 답변글이 아닌 원글인 경우 -->
					 ...
					 <a href="<%= request.getContextPath() %>/jsb/recommendView.action?seq=${boardvo.seq}&writer=${boardvo.userid}">${boardvo.subject}<span style="color: red; font-weight: bold; font-style: italic; font-size: smaller; vertical-align: super; ">[${boardvo.commentCount}]</span></a>
					
					 </c:if>
					 
					 <c:if test="${boardvo.commentCount == 0}">
					 <a href="<%= request.getContextPath() %>/jsb/recommendView.action?seq=${boardvo.seq}&writer=${boardvo.userid}">${boardvo.subject}</a>
				...
				<td align="center">${boardvo.rLikeCnt }</td>
				<td align="center">${boardvo.rDisLikeCnt }</td>
	...
	<br/>
	<button class="btn" type="button" onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/recommendList.action'">글목록</button>&nbsp;
	<button class="btn" type="button" onClick="javascript:location.href='<%= request.getContextPath() %>/jsb/recommendAdd.action'">글쓰기</button>
	
	
		
	<c:forEach var="boardvo" items="${boardList}" varStatus="status">
	 <c:forEach items="${boardvo.resultLike}" var="resultLike1">
		   <fmt:parseNumber var="i" type="number" value="${resultLike1}" />
		   		
		        <c:forEach items="${boardvo.resultLike}" var="resultLike2">
		            <fmt:parseNumber var="j" type="number" value="${resultLike2}" />
...
<!-- 
	<div id="displayList" style="width:302px; margin-left: 3px; border: solid 0px gray; border-top: 0px;">
