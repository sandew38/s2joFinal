...
public interface InterKhxService {
	...
	List<String> getseatList(HashMap<String, Object> map); // 팔린 좌석 목록 가져오기 (좌석번호별...이거안됨)
	
	List<String> gettakenseatList(HashMap<String, Object> map); // 팔린 좌석 목록 가져오기 

	int loginEnd(HashMap<String, String> map);	//	yj 회원 로그인 여부 확인

	khxpscMemberVO getLoginEnd(String userid); // yj 로그인한 회원 아이디 가져오기

	int nonloginEnd(HashMap<String, String> map); // yj 비회원 로그인 여부 확인

	khxpscNonMemberVO getNonLoginEnd(String nhp); // yj 비회원 정보 가져오기
